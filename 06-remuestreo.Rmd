...
Sin embargo, el intervalo del error éstandar está construido bajo el supuesto
que el TCL es una buena aproximación. Anteriormente vimos que este supuesto
parece ser cierto en este ejemplo. Aunque también notamos un grado de asimetría
en el histograma. ¿Cómo se traduce está asimetría en la calidad del intervalo de
confianza? Empecemos por el intervalo basado en el error éstandar. Claramente
éste no está bien calibrado, como podemos notar en los resúmenes de cobertura
para nuestras remuestras bootstrap:

```{r, echo = FALSE}
totales_boot %>% 
  mutate(upper = total_boot >= max(intervalo_total * 1000), 
         lower = total_boot <= min(intervalo_total * 1000)) %>% 
  summarise(cobertura_inf = mean(lower), 
            cobertura_sup = mean(upper))
```

los cuales se han calculado como el porcentaje de medias bootstrap por debajo
(arriba) de la cota inferior (superior).

En comparación, el intervalo basado en percentiles está construido para
satisfacer está garantía (ver definición). Más aún, podemos notar que el
intervalo bootstrap de percentiles construye un intervalo asímetrico con
respecto a la media. Ilustramos esto con la distancia de las extremos del
intervalo con respecto a la media:
