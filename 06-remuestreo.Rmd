...
```{r}
sims_tbl <- sims_intervalos %>% bind_rows
ggplot(sims_tbl, aes(x = rep)) +
  geom_hline(yintercept = sum(poblacion_casas$precio_miles), colour = "red") +
  geom_linerange(aes(ymin = inf, ymax = sup)) 
```
La cobertura para estos 100 intervalos simulados da

```{r}
total <- sum(poblacion_casas$precio_miles)
sims_tbl %>% mutate(cubre = inf < total & total < sup) %>% 
  summarise(cobertura = mean(cubre))
```
que es consistente con una cobertura real del 95%.
