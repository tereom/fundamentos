...
como distribución inicial para $\mu ~ N(\m_0,\sigma_0)$, y definimos
como al precisión $\tau$ como el inverso de la varianza, entonces la posterior
...
- Sean $X_i\sim N(\mu, \sigma)$. Queremos estimar $\mu$ y $\sigma$. Tomamos
...
nacional en estados unidos está alrededor de 1.75, y el percentil 90% es 1.82. Quizá los
...
poco creíbles, pero no extremadamente.
...
ggplot(sims %>% filter(dist == "posterior"), 
       aes(x = mu, y = sigma)) + geom_point(colour = "#00BFC4") +
  scale_x_continuous(breaks = c(175, 176)) +
  coord_equal()
```
Los estimadores de máxima verosimlitud son

```{r}
mean(cantantes$estatura_cm)
sd(cantantes$estatura_cm)
```
