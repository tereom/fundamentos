...
$$P(\theta| X_1=x_1, \ldots, X_n=x_n) \propto P(X_1 = x_1,\ldots X_n=x_n | \theta)P(\theta) = \theta^{k+1}(1-\theta)^{n-k + 1}$$
...
como distribuciones iniciales:
  ...
  - $\mu\sim \mathsf{N}(\mu_0, \sigma_0)$ , y
  - $\tau = \frac{1}{\sigma^2} \sim \mathsf{Gamma}(\alpha,\beta )$
  - $p(\mu, \sigma) = p(\mu)p(\sigma)$

- Entonces la posterior es:
  - $\mu$ es normal con media $\mu' = \frac{\tau_0\mu_{0}+n{\bar{x}}}{\tau_0 +n}$ y precisión $\tau' = \tau_0 +n$.
  - $\tau = \frac{1}{\sigma^2}$ es $\mathsf{Gamma}(\alpha', \beta')$, con $\alpha' = \alpha + n/2$, 
  $\beta' = \beta + \frac{1}{2}\sum_{i=1}^{n}(x_{i} - \bar{x})^2 + \frac{n\tau_0}{\tau_0 + n}\frac{({\bar{x}}-\mu_{0})^2}{2}$
  - $p(\mu,\sigma|x) = p(\mu|x)p(\sigma|x)$
