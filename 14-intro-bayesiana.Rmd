...
Si $X_{nueva}$ es una tirada adicional de la moneda que estamos usando, nos
interesaría saber:
$$P(X_{nueva}=\mathsf{sol}|X=2)$$
Que podemos calcular observando que $P(X_{nueva}|X=2, \theta)$ es una variable 
...
$$P(X_{nueva}=\mathsf{sol}|\theta=0.4, X=2)P(\theta=0.4|X=2) +P(X_{nueva}=\mathsf{sol}|\theta = 0.6, X=2)P(\theta =0.6|X=2)$$
Es decir, promediamos ponderando con las probabilidades posteriores. Obtendríamos entonces
$$P(X_{nueva} = \mathbf{sol}) =  0.4 ( 0.31) + 0.6 (0.69) = 0.538$$
...
Bernoulli. Sabemos que el orden en que ocurren los éxitos no importa, así
que podemos reducir el problema considerando $S_n = X_1 + X_2 +\cdots, X_n$, 
el número de éxitos que
observamos en las $n$ pruebas. Ya sabemos calcular la 
verosimilitud (el modelo de los datos):

$$P(S_n=k|\theta) = {n \choose k} \, \theta^k(1-\theta)^{n-k},$$

es decir, la suma de éxitos es Binomial con parámetros $n$ y $\theta.$ 
...
$$P(\theta|S_n=k) \propto P(S_n = k | \theta)P(\theta) = \theta^{k+1}(1-\theta)^{n-k + 1}$$
...
- (Beta-binomial) Si las observaciones $X_i$ son $\mathsf{Binomial}(n, p)$ ($n$ fija)
 queremos estimar $p$, y tomamos como distribución inicial para $p$ una $\mathsf{Beta}(a,b)$,
entonces la posterior para $p$ cuando $X=k$ es $\mathsf{Beta}(k + a, n - k + b)$. 
...
- Entonces la posterior es:
  ...
  - $\mu \sim \mathsf{N}\left (\frac{\tau_0\mu_{0}+n{\bar{x}}}{\tau_0 +n}, \tau_0 +n \right )$
  - $\tau = \frac{1}{\sigma^2} \sim \mathsf{Gamma} \left (\alpha + n/2, \beta + \frac{1}{2}\sum_{i=1}^{n}(x_{i} - \bar{x})^2 + \frac{n\tau_0}{\tau_0 + n}\frac{({\bar {x}}-\mu_{0})^2}{2}\right )$
...
- Muestreamos un valor $\tilde{\theta} \sim p(\theta|x)$.
- Simulamos del modelo de las observaciones $\tilde{x} \sim p(x|\tilde{\theta})$.
  
Si queremos una muestra iid de observaciones de la predictiva posterior,

- Muestreamos un valor $\tilde{\theta} \sim p(\theta|x)$.
- Simulamos del modelo de las observaciones $\tilde{x}_1, \tilde{x}_2,\ldots, \tilde{x}_n \sim p(x|\tilde{\theta})$.
```
