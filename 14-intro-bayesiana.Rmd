...
Bernoulli con probabilidad $\theta$, que puede valer 0.4 o 0.6. Como tenemos las probabilidades
posteriores $P(\theta|X=2)$ podemos usar probabilidad total, todo condicionado a $X=2$:

$$P(X_{nv}=\mathsf{sol}|\theta=0.4, X=2)P(\theta=0.4|X=2) +P(X_{nv}=\mathsf{sol}|\theta = 0.6, X=2)P(\theta =0.6|X=2)$$
Es decir, promediamos ponderando con las probabilidades posteriores. Obtendríamos entonces
...
usando cálculo, pero recordamos que una distribución $\mathsf{\mathsf{Beta}}(a,b)$ tiene como fórmula
$$\frac{1}{B(a,b)} \theta^{a-1}(1-\theta)^{b-1}$$
Concluimos entonces que la posterior tiene una distribución $\mathsf{Beta}(22, 14)$. Podemos
simular de la posterior usando código estándar para ver cómo luce:
...
como distribuciones iniciales:
  ...
  - $\mu|\tau$ es normal con media $\mu_0$ y precisión $\tau n_0$ , y
  - $p(\mu, \tau) = p(\mu|\tau)p(\tau)$

- Entonces la posterior es:
  - $\tau|x = \frac{1}{\sigma^2}$ es $\mathsf{Gamma}(\alpha', \beta')$, con $\alpha' = \alpha + n/2$, 
  $\beta' = \beta + \frac{1}{2}\sum_{i=1}^{n}(x_{i} - \bar{x})^2 + \frac{nn_0}{n+n_0}\frac{({\bar{x}}-\mu_{0})^2}{2}$
  - $\mu|\tau,x$ es normal con media $\mu' = \frac{n_0\mu_{0}+n{\bar{x}}}{n_0 +n}$ y precisión $\tau' = \tau(n_0 +n)$.
  
  - $p(\mu,\tau|x) = p(\mu|x,\tau)p(\tau|x)$


**Observaciones.** Nótese que este último ejemplo tienen más de un parámetro. En estos casos,
el objeto de interés es la **posterior conjunta** de los parámetros $p(\theta_1,\theta_2,\cdots, \theta_p|x)$.
Estos dos ejemplos son simples pues por la selección de iniciales, los parámetros resultan ser
condicionalmente independientes, con distribución conocida, de forma que podemos simular de cada uno
por separado sin dificultad.
...
estándar sea de más de 12 centímetros.
...
```{r}
tau_0 <- 0.02 # media de la inicial de tau
mu_0 <- 175
n_0 <- 5 # cuánta precisión en la inicial
sigma_0 <- 1/sqrt(n_0*tau_0)
qnorm(c(0.05, 0.95), mu_0, sigma_0) %>% round(2)
```

Ahora tenemos que poner una inicial para la desviacion estándar. 

Podemos checar nuestros supuestos simulando posibles muestras usando
sólo nuestra información previa:

```{r, fig.width = 7, fig.height = 6}
simular_muestra <- function(n, mu_0, n_0, a, b){
  # simular media
  tau <- rgamma(1, a, b)
  mu <- rnorm(1, mu_0, 1/sqrt(n_0 * tau))
  # simular sigma
  sigma <- 1/sqrt(tau)
  rnorm(n, mu, sigma)
}
...
sims_tbl <- tibble(rep = 1:20) %>% 
  mutate(estatura = map(rep, ~ simular_muestra(500, mu_0, n_0, a, b))) %>% 
...
```{r}
calcular_pars_posterior <- function(x, mu_0, n_0, a_0, b_0){
  n <- length(x)
  media <- mean(x)
  # precisión mu
  mu <- (n_0*mu_0 + n*media) / (n + n_0)
  n_1 <- n + n_0
  sigma <- sqrt(1 / tau)
  # media mu
  ...
  # posterior sigma
  a <- a_0 + 0.5 * n
  b <- b_0 + 0.5 * sum( (x - media)^2) + 0.5 * (n*n_0) / (n + n_0) * (media - mu_0)^2
  c(mu, n_1, a, b)
}
...
simular_normal_gamma <- function(n, pars){
  ...
  # simular media
  tau <- rgamma(n, a, b)
  mu <- rnorm(n, mu_0, 1/sqrt(n_0 * tau))
  # simular sigma
  sigma <- 1/sqrt(tau)
  tibble(mu = mu, sigma = sigma)
}
sims_inicial <- simular_normal_gamma(5000, c(mu_0, n_0, a, b)) %>% 
  mutate(dist = "inicial")
sims_posterior <- simular_normal_gamma(5000, pars_posterior) %>% 
...
ggplot(sims %>% filter(dist == "posterior"), 
  ...
  scale_x_continuous(breaks = c(175, 176, 177)) +
...
muestra posible como sigue:

```{block2, type='mathblock'}
La distribución **predictiva posterior** genera nuevas observaciones a partir de
la información observada. La de notamos como $p(\tilde{x}|x)$. Para simular de ella:
...
Si queremos una muestra iid de observaciones de la predictiva posterior,
...
```{r}
simular_obs_normal_gamma <- function(n, pars){
  mu_0 <- pars[1]
  no_0 <- pars[2]
  a <- pars[3]
  b <- pars[4]
  ...
  tau <- rgamma(1, a, b)
  # simular media
  mu <- rnorm(1, mu_0, n_0 * tau)
  sigma <- 1/sqrt(tau)
  # Simular observaciones iid
  tibble(estatura_cm = rnorm(n, mu, sigma))
}
```

Los parámetros posteriores son
```{r}
names(pars_posterior) <- c("m", "sigma", "a", "b")
pars_posterior
```
...
```{r}
muestra_sim <- simular_obs_normal_gamma(20, pars_posterior) 
muestra_sim
```
...
sims_obs <- tibble(.n = 1:19) %>% 
  mutate(muestra = map(.n, ~ simular_obs_normal_gamma(20, pars_posterior))) %>% 
  unnest(muestra)
set.seed(9921)
pos <- sample(1:20, 1)
lineup_tbl <- lineup(true = cantantes %>% select(estatura_cm), 
                     samples = sims_obs, pos = pos)
ggplot(lineup_tbl, aes(x = estatura_cm)) + geom_histogram(binwidth = 2.55) +
  facet_wrap(~.sample)
```
...
En algunos casos, la posterior predictiva tiene una forma conocida, y se puede
simular directamente de ésta (ver notación arriba). Por ejemplo:

1. (Beta-Bernoulli) La posterior predictiva es Bernoulli con parámetro $p = \frac{a'}{a'+b'}$,
donde $(a',b')$ son los parámetros de la beta posterior.
2. (Normal-Gamma inverso) Lo posterior predictiva es $t$ con $2\alpha'$ grados de libertad,
centrada en $\mu'$ y escalada por $\frac{\beta'}{\alpha'}\frac{\tau' + 1}{\tau'}$

**Ejemplo** (posterior predictiva de Pareto-Uniforme).
Para el modelo Pareto-Uniforme no hay nombre para la forma de su predictiva posterior, 
pero podemos calcularla analíticamente o aproximarla por
simulación. Usando los mismos datos del ejercicio de la lotería, haríamos:
