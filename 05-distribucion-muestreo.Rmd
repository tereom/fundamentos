...
knitr::opts_chunk$set(echo = TRUE, message = FALSE, warning=FALSE, fig.align = 'center', fig.height=3)
...
su **error estándar**, y la denotamos por $\textsf{ee}(T)$. A cualquier estimador
de este error estándar lo denotamos como $\hat{\textsf{ee}}(T)$.
...
¿Cuál es la distribución de muestreo de la suma $S_n = X_1 + \cdots + X_n$? Sabemos que la suma
de exponenciales independientes es una distribución gamma con parámetros $(n, \lambda)$,
y esta es la distribución de muestreo de nuestra estadística $S_n$ bajo las hipótesis
que hicimos.
...
$$\bar{X}_n \xrightarrow{} \mathsf{N}\left( \mu, \frac{\sigma}{\sqrt{n}} \right)$$
...
- El error estándar de $\bar{X}_n$ es
$\textsf{ee}(\bar{X}_n) = \frac{\sigma}{\sqrt{n}}$. Si tenemos una muestra, podemos
estimar $\sigma$ con de la siguiente forma:
$$\hat{\sigma}_n =\sqrt{\frac{1}{n}\sum_{i=1}^n (X_i - \bar{X})^2}$$
o el más común (que explicaremos más adelante)
$$\hat{s}_n = \sqrt{\frac{1}{n-1}\sum_{i=1}^n (X_i - \bar{X})^2}$$
...
aproximación del Teorema central del límite ($\textsf{TCL}$) sea buena siempre y cuando
...
variable con distribución normal $X \sim \mathsf{N}(\mu, \sigma)$
sólo tenemos que saber los cuantiles de la distribución normal estándar $Z \sim \mathsf{N}(0,1)$ y escalarlos
...
```
Y vemos que en efecto el $\textsf{TCL}$ aplica en este ejemplo, y la aproximación es buena.
...
el $\textsf{TCL}$, de la siguiente forma:

Si $S_{30}=X_1 + X_2 + X_{30}$, entonces la media de $S_{30}$ es 15 (¿cómo se calcula')
...
tibble(n_sim = 1:100000) %>% 
  ...
  summarise(prob_may_18 = mean(suma > 18))
```
Y vemos que la aproximación normal es buena para fines prácticos.

```{block, type='ejercicio'}
Usando simulaciones haz un histograma que aproxime la distribución de muestreo de $S_n$. 
...
```{r}
sims_gamma <- tibble(n_sim = 1:2000) %>% 
  mutate(suma = map_dbl(n_sim, ~ mean(rgamma(30, 0.1, 1))))
...
- Es importante notar que la calidad de la aproximación del $\textsf{TCL}$ depende de características
de la población y también del tamaño de muestra $n$. Para ver si el $\textsf{TCL}$ aplica, podemos hacer ejercicios de simulación bajo diferentes supuestos acerca de la población. 
También veremos más adelante, con remuestreo, maneras de checar si es factible el $\textsf{TCL}$ dependiendo del análisis de una muestra dada que tengamos.

- El $\textsf{TCL}$ era particularmente importante en la práctica antes de que pudiéramos
...
exactas, particularmente en la construcción de intervalos de confianza, por ejemplo. Dependemos menos de **resultados asintóticos**, como el $\textsf{TCL}$.
