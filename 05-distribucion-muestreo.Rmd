...
y $T = t(X_1, X_2, \ldots, X_N)$ una estadística. La **distribución de muestreo** 
de $T$ es la función de distribución de $T$.
```
...
el **error estándar**, y la denotamos por $\text{se}(T)$. A cualquier estimador
de este error estándar lo denotamos como $\hat{\text{se}}(T)$.
```
...
Para tamaño de muestra $N = 100$, obtenemos más dispersión:
...
Existen tres formas de calcular la distribución de muestreo y errores estándar 
---cálculos exactos, simulación y approximación a fórmulas. 

Recordemos el ejemplo para $X_n \sim U[0,1],$ sabemos que si $0 \leq a \leq 1,$ entonces 
$P(X_n \leq a) = a.$ Por lo tanto, para $T_2 = \max_n X_n,$ la función de distribución es
\begin{align}
P(T_2 \leq a) &= P(\max_n X_n \leq a) \\
&= P(X_1 \leq a, \ldots, X_N \leq N) \\
&= P(X_1 \leq a) \cdots P(X_N \leq a) \\ 
&= a^{N}.
\end{align}

```{block, type = 'ejercicio'}
¿Cuál es la función de densidad del máximo? ¿Cuál es la función de distribución del mínimo? 
¿Cuál es la función de densidad del mínimo?
```

El resultado más notable para calcular *distribuciones* de muestreo por aproximaciones es por 
medio del *teorema de límite central,*  y lo veremos en el próximo bloque.
