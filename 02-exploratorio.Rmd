...
```{r, fig.width=3.4, fig.height=3.2}
tricubo <- function(x) {
...
El modelo de *regresion lineal* ajusta una recta a un conjunto de datos. Por ejemplo, 
consideremos la familia $f_{a,b} = a x + b$ para un conjunto de datos bivariados 
$\{ (x_1, y_1), \ldots, (x_N, y_N)\}$. La recta se ajusta al minimizar
la suma de errores cuadráticos
\begin{align}
\frac1N \sum_{n = 1}^N ( y_n - a x_n - b)^2. 
\end{align}

Sin embargo, este modelo es muy *inflexible*. Una manera de mejorar su ajuste es 
considerar rectas de manera local. Esto lo logramos al minimizar una suma de errores
cuadráticos ponderados por los pesos $w_n(x)$. Para esto minimizamos las $N$ 
funciones de error
\begin{align}
...
```{block, type = 'mathblock'}
Notemos que el ajuste tradicional por medio de mínimos cuadrados considera un ajuste
con pesos 
\begin{align}
w_n(x) = \frac1N. 
\end{align}
...
```{block, type = 'mathblock'}
En la práctica, los pesos $w_n(\cdot)$ contienen a su vez un conjunto de parámetros 
...
```{block, type = 'mathblock'}
Una generalización sencilla es considerar polinomios para el ajuste
\begin{align}
