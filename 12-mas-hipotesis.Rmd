...
$$W = \frac{\hat{\theta}_n - 125}{\hat{\textsf{ee}}_n} \sim \mathsf{N}(0,1)$$
...
de 454 (matemáticas en 6o. grado). Tomamos una muestra de 200
...
$$\frac{\hat{\theta}_n - \theta_0}{\hat{\textsf{ee}}_n} \sim \mathsf{N}(0,1).$$ 
...
$$\hat{\textsf{ee}}_n = s_n / \sqrt{n}$$
donde
$$s_n = \frac{1}{n}\sum_{i=1}^n (D_i - \bar{D})^2$$
...
$$\lambda = 2\left[\ell(\hat{p}_n) - \ell(p_0)\right] = 2[\ell(0.625)- \ell(0.5)] = 2(3.79)=7.58$$
...
$$\lambda = 2\log\left( \frac{\max_{\theta}\mathcal{L}(\theta)}{\mathcal{L}(\theta_0)}        \right ) = 2\log\left(  \frac{\mathcal{L}(\hat{\theta}_n)}{\mathcal{L}(\theta_0)}  \right)$$
...
hipótesis nula y finalmente 3) Ver qué tan extremo es el valor obtenido de $\lambda$  
en relación a la distribución de referencia.
...
$$\lambda = 2\log\left( \frac{\max_{\theta}\mathcal{L}(\theta)}{\max_{\theta\in\Theta_0}\mathcal{L}(\theta)}        \right ) = 2\log\left(  \frac{ \mathcal{L}(\hat{\theta}_n)}{\mathcal{L}(\hat{\theta}_0)}  \right)$$

  donde $\hat{\theta}_n$ es el estimador de máxima verosimilitud de $\theta$
...
$T(x) = \frac{\hat{p_1} - 0.5}{\hat{\textsf{ee}}_n}$. Podríamos definir la región
