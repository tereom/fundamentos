...
**Definición.** Denotamos por $H_0$ a la *hipótesis nula* la cual usualmente tratamos como la afirmación del *status quo.*
La hipótesis alternativa la denotamos por $H_1$ y representa el supuesto que está a prueba y para el cual buscamos evidencia en los datos.
```
...
que observamos son artificio de variación muestral, por ejemplo.**
...
En general, para realizar pruebas de hipótesis se necesitan dos ingredientes fundamentales: una medida númerica 
de la evidencia de los datos y asociar la probabilidad de que tal evidencia sea generada de manera aleatoria 
por los datos. 

```{block, type='mathblock'}
**Definición.** Una *estadística de prueba* es una función numérica de los datos cuyo valor determina el resultado de la
prueba. La función usualmente es denotada por $T(\bf X)$ donde $\bf X$ representa los datos como variable aleatoria. Por ejemplo, $T = T(X_1, \ldots, X_N)$ si sólo tenemos una muestra, o por $T = T(X_1, \ldots, X_N, Y_1, \ldots, Y_M)$ en el caso de tener dos muestras. Al evaluar la prueba para un conjunto de datos dado, $x$, ésta se denomina *estadística de prueba observada,* $t = T(x).$
```
...
grafica_cuantiles(reps_mezcla, grupo_1, x) +
    ...
    coord_flip() + ylab("x") +
    labs(caption = "Mediana y percentiles 25% y 75%")+ 
...
```{block, type='mathblock'}
**Definición.** Un resultado es *estadisticamente significativo* si tuviera muy poca probabilidad de suceder al azar.
```

## Comparaciones con lineup (Ver. 2) {-}
...
permutaciones_est <- function(datos, variable, calc_diferencia, n = 100){
...
```{r, cache=TRUE}
valores_ref <- permutaciones_est(te_azucar, how, calc_diferencia, n = N_rep)
```
...
```{r}
set.seed(2)
valores_ref <- permutaciones_est(te_azucar, Tea, calc_diferencia_2, n = N_rep)
```
...
ejemplo$media_dif <- valor_p(fusion %>% sample_frac(0.95), nv.vv, calc_dif, n = 100)
calc_dif <- calc_fusion(mean, log, VV - NV)
ejemplo$media_dif_log <- valor_p(fusion %>% sample_frac(0.95), nv.vv, calc_dif, n = 100)
calc_dif <- calc_fusion(median, identity, VV / NV)
ejemplo$mediana_razon <- valor_p(fusion %>% sample_frac(0.95), nv.vv, calc_dif, n = 100)
calc_dif <- calc_fusion(media_cuartiles, identity, VV / NV)
ejemplo$cuartiles_razon <- valor_p(fusion %>% sample_frac(0.95), nv.vv, calc_dif, n = 100)
```
