...
2. Aceptamos or rechazamos el salto:
  ...
  - Si $\alpha = \frac{f(\theta^*)}{f(\theta^{(i)})} < 1$, entonces aceptamos con probabilidad
  $\alpha$ el salto, ponemos $\theta^{(i+1)}=\theta^*$ y regresamos a 1 para la siguiente iteración $i \leftarrow i + 1$. Si rechazamos el salto, ponemos entonces $\theta^{(i+1)}=\theta^{(i)}$ y regresamos a 1 para la siguiente iteración $i\leftarrow i + 1.$

Requerimos también que la función que propone los saltos sea simétrica: es decir,
$q(\theta^*|\theta^{(i)})$ debe ser igual a 
$q(\theta^{(i)}|\theta^*)$. Se puede modificar el algoritmo
...
En este curso, escribiremos varios métodos Markov Chain Monte Carlo
desde cero para entender los básicos de cómo funciona.
Sin embargo, **en la práctica no hacemos esto**, sino que usamos
software estándar (Stan, JAGS, BUGS, etc.) para hacer este trabajo.

**Expertos** en MCMC, métodos numéricos, y estadística a veces escriben partes de sus rutinas de simulación, y pueden lograr mejoras de 
desempeño considerables. Excepto para modelos simples, esto no
es trivial de hacer garantizando resultados correctos.
```
...
$$p_{mover}=\min\bigg\{\frac{P(\theta_{propuesta})}{P(\theta^{(i)})},1\bigg\}$$
Notemos que la distribución objetivo $P(\theta)$ no necesita estar normalizada, 
esto es porque lo que nos interesa es el cociente $P(\theta_{propuesta})/P(\theta^{(i)})$.
...
la densidad de $\mu \sim N(175, 2)$ y $\sigma \sim U[2, 20]$. Igual
...
* Una generalización del algoritmo de Metrópolis es Metrópolis-Hastings. 
    ...
    2. Para $t = 1,2,...$
        ...
        propuesta $g(\theta_{propuesta}|\theta^{(i)})$ es común  que $g(\theta_{propuesta}|\theta^{(i)})$ sea una normal centrada en 
        $\theta^{(i)}$.
    3. Calculamos 
    $$p_{mover}=min\bigg( \frac{p(\theta_{propuesta})}{p(\theta^{(i)})},1\bigg)$$
    ...
    para todo $\theta^{(i)}$ y $\theta_{propuesta}$, así como simular de la 
    distribución propuesta $g(\theta_{propuesta}|\theta^{(i)})$, adicionalmente
...
$$p_{mover}=min\bigg\{ \frac{p(\theta_{propuesta})/g(\theta_{propuesta}|\theta^{(i)})}{p(\theta^{(i)})/g(\theta^{(i)}|\theta_{propuesta})},1\bigg\}$$
...
**Convergencia a estado límite**. Para determinar la convergencia  
es conveniente realizar **más de una cadena**:  buscamos ver si 
realmente se ha olvidado 
el estado inicial, si las distribuciones de cada cadena son consistentes unas con otras, y revisar  que algunas cadenas no hayan quedado *atoradas* en regiones inusuales del espacio de parámetros.
...
Y este resultado se ve mejor. La parte *transiente* hacia las zonas
