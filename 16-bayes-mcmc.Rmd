...
crear_posterior <- function(x = datos, m_0, n_0, a, b){
  ...
  posterior <- function(mu, sigma){
      ...
      tau <- 1 / sigma^2
      log_inicial <- dgamma(tau, a, b, log = TRUE) + 
        dnorm(mu, mu_0, sigma/sqrt(n_0))
...
simulaciones <- metro_normal(c(mu = 172, sigma = 3), 20000) %>% transpose() %>% as_tibble
...
```{r}
ggplot(sim_tbl, aes(x = mu, y = sigma)) + 
