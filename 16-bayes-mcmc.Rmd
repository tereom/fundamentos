...
```{r}
crear_metropolis <- function(fun, sigma_salto = 0.1){
  ...
  iterar_metropolis <- function(theta_inicial, n){
    ...
    for(i in 2:n){
      ...
      cociente <- fun(theta_prop) / fun(theta)
...
```{r}
f_dist <- function(x) 210 * dgamma(x, 20, 100)
iterador_metro_chico <- crear_metropolis(f_dist, sigma_salto = 0.001)
...
```{r}
set.seed(831)
iterador_metro_grande <- crear_metropolis(f_dist, sigma_salto = 20)
...
```{r}
set.seed(831)
iterador_metro_apropiada <- crear_metropolis(f_dist, sigma_salto = 0.1)
...
```{r}
crear_posterior <- function(x = datos, m_0, n_0, a, b){
  # calcula log_posterior
  posterior <- function(mu, sigma){
...
p <- crear_posterior(cantantes$estatura_cm, mu_0, n_0, a, b) 
post <- function(pars) { p(pars[1], pars[2]) }
```
...
simulaciones <- metro_normal(c(mu = 172, sigma = 3), 50000) %>% transpose() %>% as_tibble
...
Arriba repetimos el análisis conjugado usando Metropolis. Pero
ya no es necesario usar el modelo conjugado, y podemos poner
iniciales que sean más intuitivas y acorde con nuestr
...
Escribir
la posterior es fácil

```{r}
crear_posterior <- function(x, m_0, sigma_0, inf, sup){
  # calcula log_posterior
  posterior <- function(mu, sigma){
      log_verosim <- sum(dnorm(x, mu, sigma, log = TRUE))
      log_inicial <- 
...
```{r}
p <- crear_posterior(cantantes$estatura_cm, 175, 3, 2, 20) 
post <- function(pars) { p(pars[1], pars[2]) }
```
...
```{r}
crear_posterior <- function(x){
 
  posterior <- function(theta_azar, theta_corr){
...
```{r}
p <- crear_posterior(muestra)
post <- function(pars) { p(pars[1], pars[2]) }
set.seed(8231)
metro_examenes <- crear_metropolis(post, sigma_salto = 0.02)
...
for (j in seq(2, pasos, 2)) {
  ...
  a <- n / 2 + alpha
  b <- sum((x  - mu) ^ 2) / 2 + beta
  camino[j, "sigma"] <- sqrt(1/rgamma(1, a, b)) # Actualizar sigma
  camino[j, "mu"] <- camino[j-1, "mu"]
  # mu
  sigma <- camino[j, "sigma"]
  media <- (n * sigma_0^2 * mean(x) + sigma^2 * m) / (n * sigma_0^2 + sigma^2)
  varianza <- sigma^2 * sigma_0^2 / (n * sigma_0^2 + sigma^2)
  camino[j+1, "mu"] <- rnorm(1, media, sd = sqrt(varianza)) # actualizar mu
  camino[j+1, "sigma"] <- camino[j, "sigma"]
}
caminata <- data.frame(pasos = 1:pasos, mu = camino[1:pasos, "mu"], 
  sigma = camino[1:pasos, "sigma"])
caminata_g <- caminata %>%
