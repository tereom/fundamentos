...
Adaptado de [@Chihara]. Supongamos que una máquina produce dos tipos de bolsas de 25 galletas: la mitad de las veces produce una bolsa con 5 galletas de avena y 20 de chispas de chocolate, y
...
**Definición.** La función de verosimilitud del párametro de interés $\theta$ está definida como 
\begin{align}
\mathcal{L}(\theta; x_1, \ldots, x_n) = f(x_1; \theta) \, f(x_2; \theta) \, \cdots \, f(x_n;\theta). 
\end{align}

Esta función nos da dice qué tan creible es el valor del parámetro $\theta$ dada la muestra observada. 
...
\end{align}
donde $\theta$ denota el espacio parametral. Es decir, el espacio válido de búsqueda. 
```
...
$$l(\theta) = \log \left(\mathcal{L}_n(\theta)\right),$$
...
Supongamos que en una población de transacciones hay un porcentaje $p$ (desconocido) 
...
```{r}
curve(log_verosim, from = 0, to = 10, xname = "Porcentaje fraudulentas")
```
...
```{r}
curve(log_verosim, from = 0, to = 0.5, xname = "Porcentaje fraudulentas")
```
...
Hasta ahora hemos visto ejemplos sencillos dónde la *receta* consiste en maximizar la
...
de manera independiendte $X_i \sim U[0,b]$ para $i= 1, \ldots, n.$
...
\begin{align}
\mathcal{L}(b; x_1, \ldots, x_n) = \Pi_{i = 1}^n f(x_i; b),
\end{align}
...
de la muestra como:
\begin{align}
\mathcal{L}(b; x_1, \ldots, x_n) = \Pi_{i = 1}^n 1_{[0,b]}(x_i) \frac1b. 
\end{align}
...
$$\mathcal{L}(b; x_1, \ldots, x_n) = \left(\frac{1}{x_{\sigma(n)}}\right)^n.$$
...
optimización y métodos de búsqueda local por descenso en gradiente podrían ser ineficientes.
