...
```{r}
set.seed(41852)
muestra <- rnorm(150, mean = 1, sd = 0.5)
```
...
```{r}
res <- optim(c(0, 0.5), log_p, control = list(fnscale = -1, maxit = 10000), method = "Nelder-Mead")
res$convergence
c(media_hat = res$par[1], sigma_hat = res$par[2]) %>% round(4)
```
...
c(media_muestra = mean(muestra), sd_muestra = sd_n(muestra)) %>% round(4)
```
