...
muestra_bootstrap <- simular(length(muestra), 
...
# simular datos
set.seed(12)
muestra <- simular_modelo(200, params = logit(c(0.3, 0.28)))
qplot(muestra)
```
...
set.seed(8934)
reps_boot <- map(1:1000, ~ rep_boot(.x, simular_modelo, crear_log_p, est_mv, n = 200)) %>% 
  bind_rows
...
```{r}
ggplot(reps_boot %>% filter(convergence == 0), 
...
sucede por un problema en el planteamiento del problema, independientemente de los datos.
