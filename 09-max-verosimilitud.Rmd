...
ggplot(reps_boot, aes(sample = estimador_boot)) + 
  ...
  facet_wrap(~parametro, scales = "free_y")
```
Donde vemos que la distribución de $\sigma$ tienen sesgo a la derecha. Podemos
...
```{block, type = 'mathblock'}
Supongamos que tenemos una muestra iid $X_1,X_2,\ldots, X_n \sim f(x;\theta)$ de un modelo
